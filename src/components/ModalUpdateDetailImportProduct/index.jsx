...
const ModalUpdateDetailImportProduct = ({
    indexItem,
    item = {
        batchID: '',
        productID: '',
            ...
            <div className={cx('wrapper-detail-import-product-content')}>
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                if (new Date(e.target.value) >= Date.now()) {
                                    ...
                                    updateCellData(indexItem, { ...item, manufactureDate: e.target.value });
                                }
                            }}
                        />
                    </div>
                ...
                <div className={cx('row')}>
                    <div className={cx('form-group')}>
                        ...
                        <input type="number" placeholder="Số lượng yêu cầu" value={item.requestAmount} readOnly />
                    </div>
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                if (raw === '') {
                                    ...
                                    if (errorAmount < 0) {
                                        toast.error(
                                            'Số lượng thực tế vượt mức yêu cầu ' + item.requestAmount,
                                            styleMessage,
                                        );
                                    } else {
                                        fetchData();
                                        item.realAmount = isNaN(num) ? undefined : num;
                                        if (hasProposal) item.errorAmount = errorAmount;
                                        updateCellData(indexItem, item);
                                    }
                                }
                            }}
                ...
                <div className={cx('action-modal')}>
                    ...
                    <Button outline onClick={handleApplyDetail}>
                        <span>Đóng</span>
                    </Button>
                </div>
            </div>
